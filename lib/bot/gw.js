...
async function gwLookup(msg, arg1) {
    ...
    .setColor("#d43e46")
    .setFooter("KOTH Unofficial Gang Wars", "https://kothgangwars.herokuapp.com/logo.png")
    const lookupmsg = msg.channel.send(embed);

    try {
    const response = await interface.retrieve(arg1, 20000);
    console.log("Bot requested playerdata for " + arg1 + " and received playerdata for " + response[0].playername);
  ...
  .addFields(
    ...
    if (name != undefined) {
        ...
        console.log(err);
        console.log("Could not deliver player data for " + arg1);

        const errorembed = new Discord.MessageEmbed()
    .setTitle("Could not find player data for `" + arg1 + "`")
