...
async function gwLookup(msg, arg1) {
    ...
    console.log("Bot requested playerdata for " + arg1 + " and received playerdata for " + response.playername);

    name = response.playername;
    points = response.gamepoints;
    sets = response.setsplayed;
    wins = response.gamewins;
    losses = response.gamelosses;
